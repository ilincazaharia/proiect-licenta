...
class ManagerView:
    @staticmethod
    def render(simulation_service: SimulationService, doctor_service: DoctorService, user_id: int, config_sidebar_data: dict):
        """Randează dashboard-ul managerului cu cele două tab-uri: Simulare și Administrare Medici."""
        ...
        with tab_med:
            ...
            if st.button("Creează Cont Medic", use_container_width=True, type="primary"):
                if not m_nume or not m_prenume or not m_email or not m_password or not m_spec_sel:
                    ...
                    success_reg, msg_reg = doctor_service.register_doctor(
                        last_name=m_nume,
                        first_name=m_prenume,
                        email=m_email,
                        password=m_password,
                        specialty_name=m_spec_sel
                    )
            ...
            if not doctors:
                ...
                for doc in doctors:
                    ...
                    with col_d1:
                        st.write(f"**Dr. {doc.first_name} {doc.last_name}** - {doc.email}")
                    with col_d2:
                        st.write(role_label)
                    with col_d3:
